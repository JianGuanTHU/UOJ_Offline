...
argp_option run_program_argp_options[] =
{
	...
	{"allow-write-data"   , 505, 0             , 0, "Allow mkdir, rmdir... etc."                            , 15},
	{"add-writable-raw"   , 506, "FOLDER"      , 0, "Add a writable (don't transform to its real path)"     , 16},
	{0}
};
...
void run_child() {
	set_limit(RLIMIT_CPU, run_program_config.time_limit * 4 + 10, run_program_config.time_limit * 4 + 15);
...
RunResult trace_children() {
	...
	if (rp_timer_pid == -1) {
		...
		struct timespec ts;
		ts.tv_sec = run_program_config.time_limit * 4 + 15;
		ts.tv_nsec = 0;
		nanosleep(&ts, NULL);
		exit(0);
	}
